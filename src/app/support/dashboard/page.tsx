...
export default function SupportDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080808', fontFamily: 'system-ui, sans-serif', fontSize: '18px', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '0.875rem 1.5rem', borderBottom: '1px solid rgba(212,175,55,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#0a0a0a' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div>
            <p style={{ fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', color: '#D4AF37', margin: 0, fontWeight: 600 }}>DMS Market</p>
            <p style={{ fontSize: 17, fontWeight: 700, color: '#fff', margin: 0 }}>Centro de Soporte</p>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: 15, fontWeight: 600, color: '#fff', margin: 0 }}>{agent?.display_name}</p>
            <p style={{ fontSize: 13, color: '#D4AF37', margin: 0 }}>{agent?.role === 'support_supervisor' ? 'Supervisor' : 'Agente'}</p>
          </div>
          <button onClick={handleLogout} style={{ padding: '0.375rem 0.75rem', background: '#111', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, color: '#666', fontSize: 17, cursor: 'pointer' }}>
            Salir
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr 260px', flex: 1, height: 'calc(100vh - 64px)', overflow: 'hidden' }}>
        ...
        <div style={{ borderRight: '1px solid rgba(212,175,55,0.08)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '0.75rem', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
              style={{ width: '100%', background: '#111', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 8, color: '#fff', padding: '0.5rem', fontSize: 17, outline: 'none' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0.625rem' }}>
            {filtered.length === 0 ? (
              <p style={{ color: '#444', fontSize: 15, textAlign: 'center', marginTop: '2rem' }}>Sin tickets</p>
            ) : filtered.map(conv => (
              <div key={conv.id} onClick={() => selectConversation(conv)}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                  <p style={{ color: '#fff', fontSize: 15, fontWeight: 600, margin: 0 }}>{conv.profiles?.name ?? 'Usuario'}</p>
                  <span style={{ fontSize: 12, padding: '1px 5px', borderRadius: 999, background: `${PRIORITY_COLORS[conv.priority ?? 'media']}22`, color: PRIORITY_COLORS[conv.priority ?? 'media'], fontWeight: 700 }}>{conv.priority ?? 'media'}</span>
                </div>
                <p style={{ color: '#666', fontSize: 17, margin: '0 0 5px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{conv.subject ?? 'Consulta general'}</p>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 12, padding: '1px 7px', borderRadius: 999, background: `${STATUS_COLORS[conv.status ?? 'pendiente']}18`, color: STATUS_COLORS[conv.status ?? 'pendiente'] }}>{(conv.status ?? 'pendiente').replace('_', ' ')}</span>
                  <span style={{ fontSize: 13, color: '#444' }}>{new Date(conv.created_at).toLocaleDateString('es-CO')}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {!selected ? (
            ...
            <>
              <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <p style={{ color: '#fff', fontWeight: 600, fontSize: 18, margin: 0 }}>{selected.subject ?? 'Consulta general'}</p>
                  <p style={{ color: '#555', fontSize: 17, margin: '2px 0 0' }}>{selected.profiles?.email ?? ''}</p>
                </div>
                <div style={{ display: 'flex', gap: '0.375rem' }}>
                  {(['chat', 'perfil'] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)} style={{ padding: '0.3rem 0.75rem', borderRadius: 7, border: 'none', cursor: 'pointer', fontSize: 17, fontWeight: 600, background: tab === t ? '#D4AF37' : '#151515', color: tab === t ? '#000' : '#666' }}>
                      {t === 'chat' ? 'Chat' : 'Perfil'}
                    </button>
                  ))}
                </div>
              ...
              {tab === 'chat' ? (
                <>
                  <div style={{ flex: 1, overflowY: 'auto', padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    {messages.length === 0 && <p style={{ color: '#444', fontSize: 15, textAlign: 'center', marginTop: '2rem' }}>Sin mensajes</p>}
                    {messages.map((msg: any) => (
                      <div key={msg.id} style={{ display: 'flex', justifyContent: msg.sender_type === 'admin' ? 'flex-end' : 'flex-start' }}>
                        <div>
                          <p style={{ fontSize: 12, color: '#444', margin: '0 0 3px', textAlign: msg.sender_type === 'admin' ? 'right' : 'left' }}>
                            {msg.sender_type === 'admin' ? agent?.display_name ?? 'Asesor' : msg.sender_type === 'bot' ? 'IA' : 'Usuario'}
                          </p>
                          <div style={{ maxWidth: 380, padding: '0.5rem 0.875rem', borderRadius: 12, background: msg.sender_type === 'admin' ? 'rgba(212,175,55,0.12)' : '#161616', color: msg.sender_type === 'admin' ? '#D4AF37' : '#ccc', fontSize: 18, lineHeight: 1.5, border: msg.sender_type === 'admin' ? '1px solid rgba(212,175,55,0.2)' : '1px solid rgba(255,255,255,0.05)' }}>
                            {msg.message}
                          </div>
                        </div>
                      </div>
                  ...
                  {showCanned && cannedResponses.length > 0 && (
                    <div style={{ padding: '0.625rem 1.25rem', borderTop: '1px solid rgba(255,255,255,0.04)', background: '#0a0a0a' }}>
                      <p style={{ fontSize: 12, color: '#D4AF37', letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 0.5rem', fontWeight: 600 }}>Respuestas rapidas</p>
                      {cannedResponses.map(cr => (
                        <button key={cr.id} onClick={() => { setReply(cr.content); setShowCanned(false) }}
                          style={{ display: 'block', width: '100%', textAlign: 'left', padding: '0.4rem 0.625rem', background: '#111', border: '1px solid rgba(212,175,55,0.1)', borderRadius: 7, color: '#aaa', fontSize: 17, cursor: 'pointer', marginBottom: 3 }}>
                  ...
                  <div style={{ padding: '0.75rem 1.25rem', borderTop: '1px solid rgba(255,255,255,0.04)', display: 'flex', gap: '0.5rem', alignItems: 'flex-end' }}>
                    ...
                    <textarea value={reply} onChange={e => setReply(e.target.value)} onKeyDown={handleKey} placeholder="Escribe tu respuesta... (Enter para enviar)" rows={2}
                      style={{ flex: 1, padding: '0.5rem 0.75rem', resize: 'none', background: '#111', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 10, color: '#fff', fontSize: 18, outline: 'none', fontFamily: 'inherit' }} />
                    <button onClick={sendReply} disabled={sending || !reply.trim()}
                      style={{ padding: '0.5rem 1rem', background: sending || !reply.trim() ? '#111' : '#D4AF37', border: 'none', borderRadius: 10, color: sending || !reply.trim() ? '#444' : '#000', fontSize: 15, fontWeight: 700, cursor: sending || !reply.trim() ? 'not-allowed' : 'pointer' }}>
                      {sending ? '...' : 'Enviar'}
                    </button>
                  </div>
                </>
                  ...
                  {!userProfile ? <p style={{ color: '#444', fontSize: 12 }}>Cargando...</p> : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                      <div style={{ background: '#0f0f0f', border: '1px solid rgba(212,175,55,0.08)', borderRadius: 12, padding: '0.875rem' }}>
                        <p style={{ fontSize: 12, color: '#D4AF37', letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 0.625rem', fontWeight: 600 }}>Cliente</p>
                        <p style={{ color: '#fff', fontWeight: 600, fontSize: 17, margin: '0 0 4px' }}>{userProfile.profile?.name ?? 'Sin nombre'}</p>
                        <p style={{ color: '#777', fontSize: 15, margin: '0 0 3px' }}>{userProfile.profile?.email ?? ''}</p>
                        <p style={{ color: '#555', fontSize: 17, margin: 0 }}>Rol: <span style={{ color: '#D4AF37' }}>{userProfile.profile?.role ?? ''}</span></p>
                      </div>
                      <div style={{ background: '#0f0f0f', border: '1px solid rgba(212,175,55,0.08)', borderRadius: 12, padding: '0.875rem' }}>
                        <p style={{ fontSize: 12, color: '#D4AF37', letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 0.625rem', fontWeight: 600 }}>Resumen</p>
                        <p style={{ color: '#fff', fontSize: 24, fontWeight: 700, margin: '0 0 4px' }}>${(userProfile.totalSpent ?? 0).toLocaleString('es-CO')}</p>
                        <p style={{ color: '#777', fontSize: 15, margin: 0 }}>{userProfile.orders?.length ?? 0} ordenes</p>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div style={{ overflowY: 'auto', padding: '0.875rem' }}>
          {!selected ? (
            <p style={{ color: '#333', fontSize: 15, textAlign: 'center', marginTop: '3rem' }}>Selecciona un ticket</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
              <div style={{ background: '#0f0f0f', border: '1px solid rgba(212,175,55,0.08)', borderRadius: 12, padding: '0.875rem' }}>
                <p style={{ fontSize: 12, color: '#D4AF37', letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 0.625rem', fontWeight: 600 }}>Estado</p>
                {STATUS_OPTIONS.map(s => (
                  <button key={s} onClick={() => updateStatus(s)} style={{ display: 'block', width: '100%', padding: '0.4rem 0.625rem', borderRadius: 8, border: 'none', cursor: 'pointer', textAlign: 'left', background: selected.status === s ? `${STATUS_COLORS[s]}18` : 'transparent', color: selected.status === s ? STATUS_COLORS[s] : '#555', fontSize: 15, fontWeight: selected.status === s ? 700 : 400, borderLeft: `3px solid ${selected.status === s ? STATUS_COLORS[s] : 'transparent'}`, marginBottom: 4 }}>
                    {s.replace('_', ' ')}
                  </button>
                ))}
              </div>
              <div style={{ background: '#0f0f0f', border: '1px solid rgba(212,175,55,0.08)', borderRadius: 12, padding: '0.875rem' }}>
                <p style={{ fontSize: 12, color: '#D4AF37', letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 0.625rem', fontWeight: 600 }}>Prioridad</p>
                {PRIORITY_OPTIONS.map(p => (
                  <button key={p} onClick={() => updatePriority(p)} style={{ display: 'block', width: '100%', padding: '0.4rem 0.625rem', borderRadius: 8, border: 'none', cursor: 'pointer', textAlign: 'left', background: selected.priority === p ? `${PRIORITY_COLORS[p]}18` : 'transparent', color: selected.priority === p ? PRIORITY_COLORS[p] : '#555', fontSize: 15, fontWeight: selected.priority === p ? 700 : 400, borderLeft: `3px solid ${selected.priority === p ? PRIORITY_COLORS[p] : 'transparent'}`, marginBottom: 4 }}>
                    {p}
                  </button>
                ))}
              </div>
              <div style={{ background: '#0f0f0f', border: '1px solid rgba(212,175,55,0.08)', borderRadius: 12, padding: '0.875rem' }}>
                <p style={{ fontSize: 12, color: '#D4AF37', letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 0.625rem', fontWeight: 600 }}>Acciones</p>
                <button onClick={() => updateStatus('resuelto')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.375rem', width: '100%', padding: '0.5rem', background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8, color: '#10B981', fontSize: 17, fontWeight: 700, cursor: 'pointer', marginBottom: 6 }}>
                  Marcar resuelto
                </button>
                <button onClick={() => updateStatus('cerrado')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.375rem', width: '100%', padding: '0.5rem', background: 'rgba(107,114,128,0.08)', border: '1px solid rgba(107,114,128,0.15)', borderRadius: 8, color: '#6B7280', fontSize: 17, fontWeight: 700, cursor: 'pointer', marginBottom: 6 }}>
                  Cerrar ticket
                </button>
                <button onClick={() => updatePriority('urgente')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.375rem', width: '100%', padding: '0.5rem', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, color: '#EF4444', fontSize: 17, fontWeight: 700, cursor: 'pointer' }}>
                  Marcar urgente
                </button>
              </div>
              <div style={{ background: '#0f0f0f', border: '1px solid rgba(212,175,55,0.08)', borderRadius: 12, padding: '0.875rem' }}>
                <p style={{ fontSize: 12, color: '#D4AF37', letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 0.625rem', fontWeight: 600 }}>Info</p>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ color: '#444', fontSize: 11 }}>ID</span>
                  <span style={{ color: '#666', fontSize: 13, fontFamily: 'monospace' }}>{selected.id?.slice(0, 8)}...</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ color: '#D4AF37', fontSize: 17, fontWeight: 600 }}>{messages.length}</span>
                </div>
              </div>
            </div>
          )}
