...
export default async function VendorOrdersPage() {
  ...
  const css = `
    ...
    .var-tag{display:inline-block;background:#1a1a00;border:1px solid rgba(212,175,55,.3);color:#D4AF37;font-size:12px;font-weight:600;padding:3px 10px;border-radius:6px;margin-right:6px;margin-top:4px;}
  `
        ...
        <div className="vo-inner">
          ...
          {!orders || orders.length === 0 ? (
            ...
            orders.map((order: any) => {
              ...
              const variantes = order.variantes_seleccionadas
                    ...
                    <div className="vo-grid2">
                      ...
                      {variantes.length > 0 && (
                        <div style={{ gridColumn: "1/-1", marginTop: 4 }}>
                          <span className="vo-label">Variantes seleccionadas: </span>
                          <div style={{ marginTop: 6 }}>
                            {variantes.map((v: any, i: number) => (
                              <span key={i} className="var-tag">{v.nombre}: {v.opcion}</span>
                            ))}
                          </div>
                        </div>
                      )}
