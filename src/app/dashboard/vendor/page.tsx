...
export default async function VendorDashboard() {
  ...
  const { data: orders } = await admin
    .from("orders")
    .select("id, status, total, seller_amount, platform_fee, created_at")
    .eq("seller_id", user.id)
    .order("created_at", { ascending: false })

  const { data: payouts } = await admin
    .from("payouts")
    .select("id, amount, status, release_date")
    .eq("seller_id", user.id)
        ...
        <div style={{ marginBottom: '2rem', borderBottom: '1px solid #f0f0f0', paddingBottom: '1.5rem', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <Link href="/dashboard/vendor/ordenes"
              style={{ background: '#fff', color: '#111', padding: '0.75rem 1.25rem', textDecoration: 'none', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', border: '1px solid #ddd', whiteSpace: 'nowrap' }}>
              Mis ordenes
            </Link>
            {isApproved && (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                <thead>
                  <tr style={{ background: '#fafafa' }}>
                    {['Orden', 'Estado', 'Total', 'Tu ganancia', 'Fecha'].map(h => (
                ...
                <tbody>
                  {orders.slice(0, 5).map((order, i) => (
                    <tr key={order.id} style={{ borderBottom: i < orders.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
                      <td style={{ padding: '0.75rem 1rem', color: '#111', fontFamily: 'monospace', fontSize: '0.75rem' }}>{order.id.slice(0, 8)}...</td>
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <span style={{ fontSize: '0.65rem', padding: '0.2rem 0.6rem', borderRadius: '999px', fontWeight: 600, background: '#f0f0f0', color: order.status === 'delivered' || order.status === 'released' ? '#2e7d32' : order.status === 'paid' ? '#92400e' : '#555' }}>
                          {order.status}
                        </span>
                      </td>
                      <td style={{ padding: '0.75rem 1rem', color: '#111', whiteSpace: 'nowrap' }}>${Number(order.total).toLocaleString('es-CO')}</td>
